...
"""
__author__ = "Cristopher Gomez, Giovanni Pais"
...
class LegSkill(object):
	...
	JOINT_NAMES_BASE = ["thigh_pitch_joint", "thigh_yaw_joint",
	  ...
	  "phalange_joint"]
	JOINT_NAMES_BASE= ['thigh_pitch_joint', 'tibia_joint','ankle_joint']
...
if __name__ == "__main__":
	...
	while not rospy.is_shutdown():
		...
		for i,thetha in enumerate(thigh_angles):
			...
			if count==20:
				count=0
				#print "Moving left leg to: [",thigh_angles[i],"] [",tibia_angles[i],"] [",ankle_angles[i],"]"
				#print "Moving right leg to: [",thigh_angles_c[i],"] [",tibia_angles_c[i],"] [",ankle_angles_c[i],"]"
				print "Moving left ankle: [",ankle_angles[i],"]"
				#print "Moving right ankle: [",ankle_angles_c[i],"]"
